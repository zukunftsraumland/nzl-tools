<template>
    ...
    <!-- Category Dropdown -->
    <select v-if="selectedCategories.length > 0 || selectedCategoryId" v-model="selectedCategoryId"
        @change="onCategoryChange">
        <option value="">Select Funding Category</option>
        <option v-for="category in selectedCategories" :key="category.id" :value="category.id">
            {{ category.name }}
        </option>
    </select>

    <!-- Article Dropdown -->
    <select v-if="selectedArticles.length > 0 || selectedArticleId" v-model="selectedArticleId"
    ...
    <!-- Method Dropdown -->
    <select v-if="selectedMethods.length > 0 || selectedMethodId" v-model="selectedMethodId">
                ...
                if (this.selectedPeriodId) {
                    ...
                    if (selectedPeriod) {
                        ...
                        if (this.selectedCategoryId) {
                            const selectedCategory = this.selectedCategories.find(c => c.id === this.selectedCategoryId);
                            if (selectedCategory) {
                                this.selectedArticles = selectedCategory.articles || [];

                                if (this.selectedArticleId) {
                                    const selectedArticle = this.selectedArticles.find(a => a.id === this.selectedArticleId);
                                    if (selectedArticle) {
